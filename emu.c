...
const type16 table_base[4] =
{0xFD38 /*Wonderland*/,
 0xFEE8 /*Corruption*/,
 0xFF06 /*Fish*/,
 0xFF0A /*Guild*/};
const type8s* gfxlist[4] =
{"bank"   /*Wonderland*/,
 "cstart" /*Corruption*/,
 "ffirst" /*Fish*/,
 "griver" /*Guild*/};
...
{
	next_table = 1; // so the "wonderland-bird-animation" can be correctly played more than once ...
...
type16 find_gfx_table(void)
{
	type16 i = 0, found = 0, res = 0;
	
	while ((i < (mem_size - 7)) && (!found))
	{
		if (strcmp((type8s*)(code + i), gfxlist[0]) == 0)
		{
			found = 1;
			res = i;
			v4_id = 0;
		}
		if (strcmp((type8s*)(code + i), gfxlist[1]) == 0)
		{
			found = 1;
			res = i;
			v4_id = 1;
		}
		if (strcmp((type8s*)(code + i), gfxlist[2]) == 0)
		{
			found = 1;
			res = i;
			v4_id = 2;
		}
		if (strcmp((type8s*)(code + i), gfxlist[3]) == 0)
		{
			found = 1;
			res = i;
			v4_id = 3;
	...
	while (1)
	{
		...
		case 1: /* folders */
			...
			for (i = 0, j = 0; i < hint->elcount; i++)
			{
				output_number(i+1);
				output_text(". ");
				j += output_text((char*)hint->content+j)+1;
				ms_putchar('\n');
			}
 ...
 */
	...
	if ((byte2 < 0xdd) || (version < 4 && byte2 < 0xe4) || (version < 2 && byte2 < 0xed))
	{
		...
		case 2:	/* A0DF */
			...
			if ((table_dist == 0) && (gfxtable == 0))
			{
				table_dist = find_gfx_table();
				gfxtable = table_base[v4_id];
			}
